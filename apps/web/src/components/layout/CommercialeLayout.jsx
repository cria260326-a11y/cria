...
import {
    LayoutDashboard, Users, FileText, Percent,
    MessageSquare, LogOut, ChevronRight, User
} from 'lucide-react';
...
const CommercialeLayout = ({ children }) => {
    ...
    const isActive = (path) => {
        if (path === '/dashboard/commerciale') return location.pathname === path;
        ...
        <div className="flex h-screen bg-background overflow-hidden">
            ...
            <aside className="w-64 flex-shrink-0 bg-card border-r border-border flex flex-col">
                <div className="h-20 flex items-center justify-center px-5 border-b border-border flex-shrink-0">
                    <Link to="/dashboard/commerciale" className="flex items-center">
                ...
                <div className="border-t border-border p-4 flex-shrink-0">
                    <Link
                        to="/dashboard/commerciale/profilo"
                    ...
                    >
                        ...
                        <div className="min-w-0 flex-1">
                            <p className="text-sm font-medium text-foreground truncate">{user?.name || 'Commerciale'}</p>
                            <p className="text-xs text-muted-foreground">Commerciale</p>
                        </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                <header className="h-16 bg-card border-b border-border flex items-center justify-between px-6 flex-shrink-0">
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        {location.pathname !== '/dashboard/commerciale' && activeItem && (
                            <>
